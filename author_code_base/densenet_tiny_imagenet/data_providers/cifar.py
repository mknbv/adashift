...
class TinyDataProvider(DataProvider):
    """Abstract class for tiny-imagenet readers"""
    ...
    def __init__(self, save_path=None, validation_set=None,
        ...
        # add train and validations datasets
        images, labels = self.read_tiny_train()
        if validation_set is not None and validation_split is not None:
            split_idx = int(images.shape[0] * (1 - validation_split))
            self.train = CifarDataSet(
                images=images[:split_idx], labels=labels[:split_idx],
        ...
        # add test set
        images, labels = self.read_tiny_test()
        self.test = CifarDataSet(
            images=images, labels=labels,
            shuffle=None, n_classes=self.n_classes,
            normalization=normalization,
    ...
    def read_tiny_test(self):

        data_X = np.load('./dataset/tiny-imagenet/tiny-imagenet_val_image_matrix.npy').astype(float)
        data_Y = np.load('./dataset/tiny-imagenet/tiny-imagenet_val_label_matrix.npy').astype(int)
        ...
        for i in range(len(data_Y)):
            if data_Y[i] < 1000:
                self.test_X.append(data_X[i])
                self.test_Y.append(data_Y[i])
        self.test_X = np.asarray(self.test_X)
        self.test_Y = np.asarray(self.test_Y)

        if self.one_hot:
            self.test_Y = self.labels_to_one_hot(self.test_Y)

        return self.test_X, self.test_Y

    def read_tiny_train(self):
        ...
        data_X = np.load('./dataset/tiny-imagenet/tiny-imagenet_image_matrix.npy').astype(float)
        data_Y = np.load('./dataset/tiny-imagenet/tiny-imagenet_label_matrix.npy').astype(int)
        data_X = data_X.transpose([3, 0, 1, 2])
        data_Y -= 1
        data_X = (data_X[:, ::2, ::2, :] + data_X[:, 1::2, ::2, :] + data_X[:, ::2, 1::2, :] + data_X[:, 1::2, 1::2, :]) / 4.
        ...
        if self.one_hot:
            self.data_Y = self.labels_to_one_hot(self.data_Y)
